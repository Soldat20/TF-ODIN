/* 
TFO_fnc_vehicleLoadout
by Fight9

Adds a predefined loadout or custom one to vehicle. 
designed for use by TFO Ammo, TFO 3M, and via script
...
examples: 
	_null = [this,1,true,true] call TFO_fnc_vehicleLoadout; // rifle loadout w/ clear cargo
	_null = [victor1,3] call TFO_fnc_vehicleLoadout; // assault loadout with initial gear left
	_null = [car1,0,true] call TFO_fnc_vehicleLoadout; // clear cargo but add nothing
	
	_loadout = [["FirstAidKit",5],["SmokeShell",10],["ItemRadio",2],["Chemlight_green",5]]; // array of arrays with classname and quantity
	_null = [vehicleName,_loadout,true,false] call TFO_fnc_vehicleLoadout; // new loadout sent to and added to vehicle
*/
...
#define preProc(A) call compile preprocessFileLineNumbers ("\TFO_ammoBoxes\vehicle\loadouts\" + A)
...
if (_logic isKindOf "TFO_ModuleVicLO") then {
...
if (typeName _selection == typeName 0) then {
	...
	if (typeName _selection == typeName []) then {
		...
		diag_log 'TFO_fnc_vehicleLoadout: wrong loadout type sent. Must be selection (0-3) or array [["classname",count],["MediKit",5]]';
	};			
};
if (count _gear > 0 && typeName (_gear select 0) != typeName []) exitWith {
	diag_log 'TFO_fnc_vehicleLoadout: Wrong gear array format sent. Must be [["classname",count],["MediKit",5]]';
};	
{
	if (_x in vehicles) then {
		...
		if (_note) then {
			...
			if (_startText != "") then {
				...
				["TFO_vicCrates",[_picture,_text]] call bis_fnc_showNotification;
			};
		};
	} else {
		diag_log format ["TFO_fnc_vehicleLoadout: Object is not a vehicle: %1",_x];
		_diag = false;
	};	
} foreach _objs;
if (isServer && _diag) then {
	diag_log format ["TFO_fnc_vehicleLoadout: Loadout Initialized on: %1",_objs];
};
true;